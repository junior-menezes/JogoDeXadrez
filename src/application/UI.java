...
public class UI {
	...
	public static XadrezPosition leiaXadrezPosition(Scanner sc) {
		try {
			...
			throw new InputMismatchException("Erro na leitura da posicao. Os dados validos sao de a1 ate h8.");
		}
		// sc.close();
	}
	...
	public static void printTabuleiro(PecaDeXadrez[][] pecas) {
		for (int i = 0; i < pecas.length; i++) {
			...
			for (int j = 0; j < pecas.length; j++) {
				printPeca(pecas.clone()[i][j], false);
			}
	...
	public static void printTabuleiro(PecaDeXadrez[][] pecas, boolean[][] possiveisMovimentos) {
		for (int i = 0; i < pecas.length; i++) {
			...
			for (int j = 0; j < pecas.length; j++) {
				printPeca(pecas.clone()[i][j], possiveisMovimentos[i][j]);
			}
